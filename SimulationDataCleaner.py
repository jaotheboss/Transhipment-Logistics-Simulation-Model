...
class SimulationDataCleaner():
       """
       function:     this class is created to clean and manipulate the raw 
                     transhipment data into whatever the simulation requires it to be
       input:        a data file
       attributes:   the data and updated with whatever methods that have been done
       methods:      cleaning and extracting of scene data (for now)
       """
       ...
       def scenarioA(self, df):
              ...
              input:        dataframe
              returns:      array of connection time (n x 1) for scenario A
              """
              l_atb = df.iloc[:, 6]
              d_atu = df.iloc[:, 5]
              ctime = l_atb - d_atu
       ...
       def scenarioB(self, df):
              ...
              input:        dataframe
              returns:      array of connection time (n x 1) for scenario B
              """
              l_atb = df.iloc[:, 6]
              d_dt = df.iloc[:, 4]
              ctime = l_atb - d_dt
       ...
       def scenarioC(self, df):
              ...
              input:        dataframe
              returns:      array of connection time (n x 1) for scenario C
              """
              l_dt = df.iloc[:, 7]
              d_dt = df.iloc[:, 4]
              ctime = l_dt - d_dt
       ...
       def clean(self, export = False):
              ...
              input:        dataframe
              returns:      cleaned dataset; dataset with all the connection time scenarios
              """
              ...
              # collating all the connection time data into one dataframe
       ...
       def fill_fake_events(self):
              """
              This method did not work and the headspace idea that's seen
              in `Class_TuasShiftingSimulation.py` is preferred.
